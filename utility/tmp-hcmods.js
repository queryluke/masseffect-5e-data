    ...
    vorchaAdaptations: [
        {
            ...
            name: 'Aquatic',
            mechanics: [
                { type: 'speed', speed: 'swim', distance: 30 },
                { type: 'model', model: 'trait', value: 'amphibious' }
            ]
        }
    ]
}
...
for (const [edgetype, edgeList] of Object.entries(edges)) {
    for (const edge of edgeList) {
        edge.type = edgetype
        const id = edge.id
        const body = edge.html || ''
        delete edge.id
        delete edge.html
        const prefix = edgetype === 'fighting-styles' ? 'FS' : edgetype === 'subgrounds' ? 'BG' : 'VA'
        const text = '---\r\n' + YAML.stringify(edge) + '\n---\r\n' + body
        fs.writeFileSync(`./data/edges/${prefix}_${id}.md`, text)
    }
